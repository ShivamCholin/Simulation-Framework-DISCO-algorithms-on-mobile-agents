...
class agent:
    ...
    def communicate(self):
        if self.follower==True and self.visible_agents and self.visible_agents[0].ID==self.following_agent_ID and self.visible_agents[0].stop_moving==True and self.visible_agents[0].ID!=self.following_agent.ID:
            self.following_agent=self.visible_agents[0]
            return
        if self.follower:
...
if __name__ == "__main__":
    ...
    simulate(poly,50,True,False,20,10)