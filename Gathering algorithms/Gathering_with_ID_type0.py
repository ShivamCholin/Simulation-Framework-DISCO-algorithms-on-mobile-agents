...
class agent:
    ...
    def process(self):
        ...
        if not self.follower:
            if self.visible_agents and self.visible_agents[0].ID<self.ID:
                self.follower=True
                self.following_agent=self.visible_agents[0]
                self.mov_next=self.following_agent.current_pos
                self.mov_next_set=True
            elif not self.round_number%2==0:
                next_mov=random_points_within(self.visibility_polygon,self.poly_tuple,1)[0]
                self.mov_next=next_mov
                self.mov_next_set=True
            else:
                next_mov=self.current_pos
                self.mov_next=next_mov
                self.mov_next_set=True
        else:
            if self.visible_agents and self.visible_agents[0].ID<self.following_agent.ID:
                self.following_agent=self.visible_agents[0]
                self.mov_next=self.following_agent.current_pos
                self.mov_next_set=True
            else:
